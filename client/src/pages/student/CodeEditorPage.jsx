...
export default function CodeEditorPage() {
  ...
  const handleLangChange = (lang) => {
    ...
    setCode(
      selectedProblem.starterCode[lang.id] ||
        CODE_TEMPLATES[lang.id] ||
        "// Write your code here",
    );
    setOutput("");
  ...
  const handleProblemChange = (problem) => {
    ...
    setCode(
      problem.starterCode[selectedLang.id] ||
        CODE_TEMPLATES[selectedLang.id] ||
        "// Write your code here",
    );
    setOutput("");
    setOutputStatus(null);
  ...
  const runCode = async () => {
    ...
    try {
      ...
      setOutput(
        "⚠️ Network error: Could not reach the code execution server.\nPlease check your internet connection and try again.",
      );
      setOutputStatus("error");
    } finally {
      setRunning(false);
    ...
    try {
      ...
      if (compileErr) {
        ...
        testCases.forEach((tc) => {
          if (
            stdout.includes(tc.expectedOutput) ||
            stdout
              .replaceAll('"', "")
              .includes(tc.expectedOutput.replaceAll('"', ""))
          ) {
            passed++;
          }
        });
      ...
      try {
        await API.post(
          `/problems/${selectedProblem._id || selectedProblem.id}/submit`,
          {
            code,
            language: selectedLang.name,
            verdict: verdict.type,
            runtime: parseFloat(elapsed),
            score:
              verdict.type === "Accepted" ? selectedProblem.score || 10 : 0,
          },
        );
      } catch (err) {
    ...
    const fetchProblems = async () => {
      try {
        ...
        if (res.data.problems?.length > 0) {
          ...
          setCode(
            res.data.problems[0].starterCode?.python ||
              CODE_TEMPLATES["python"],
          );
        }
      } catch (err) {
        console.warn("Using local problems:", err?.message);
      }
    };
    fetchProblems();
  }, []);
            ...
            {problems.map((p) => (
              <div
                ...
                style={{
                  ...
                  background:
                    selectedProblem.id === p.id ? "#1f2937" : "transparent",
                  borderLeft:
                ...
                >
                  <span
                    style={{
                      ...
                      fontWeight: 600,
                      color:
                        selectedProblem.id === p.id ? "#e6edf3" : "#c9d1d9",
                    }}
                  >
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", gap: 6 }}>
              {["description", "examples", "solution", "submissions"].map(
                (t) => (
                  <button
                    key={t}
              ...
              >
                <span
                  style={{
                    ...
                    background: DIFF_BG[selectedProblem.difficulty],
                    padding: "2px 10px",
                    borderRadius: 12,
                  }}
                >
              ...
              >
                ...
                >
                  ...
                  <p style={{ marginBottom: 10 }}>
                    1. Analyze the time & space complexity constraints before
                    implementing.
                  </p>
                  <p style={{ marginBottom: 10 }}>
                    2. Utilize HashMap or Two-Pointers pattern to optimize
                    lookup speeds.
                  </p>
                </div>
              </div>
                    ...
                    {submissionsHistory.map((sub) => (
                      ...
                      >
                        <div>
                          ...
                          >
                            {sub.language} • {sub.passed}/{sub.total} test cases
                            • {sub.time}
                          </div>
                        </div>
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <button
                onClick={() =>
                  setCode(
                    selectedProblem.starterCode[selectedLang.id] ||
                      CODE_TEMPLATES[selectedLang.id],
                  )
                }
                style={{
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
                  ...
                  {submissionVerdict && (
                    <span
                      style={{
                        ...
                        borderRadius: 4,
                        background:
                          submissionVerdict.type === "Accepted"
                            ? "rgba(35,134,54,0.2)"
                            : "rgba(248,81,73,0.2)",
                        color:
                          submissionVerdict.type === "Accepted"
              ...
              >
                {running ? (
                  ...
                  <span style={{ color: "#f0883e" }}>
                    🚀 Submitting solution &amp; running test suite...
                  </span>
                ) : (
                  output || (
                    <span style={{ color: "#484f58" }}>
                      // Click ▶ Run Code to test custom input or 🚀 Submit for
