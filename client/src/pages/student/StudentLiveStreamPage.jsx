...
export default function StudentLiveStreamPage() {
  ...
  const sendChat = async (e) => {
    ...
    // Simple local state mein add karo abhi
    setMessages((prev) => [
      ...prev,
      {
        id: Date.now(),
    ...
    >
        ...
        <div className="flex justify-between items-center flex-wrap gap-md">
          <div>
            <div className="flex items-center gap-xs mb-xs">
              ...
              {streamInfo?.subject && (
                <span className="badge badge-accent">{streamInfo.subject}</span>
              )}
            </div>
            <h2 className="text-xl font-bold">
              {streamInfo?.title || "Waiting for faculty to go live..."}
            </h2>
            {streamInfo?.hostName && (
              <p className="text-secondary text-sm">
                Instructor: {streamInfo.hostName}
              </p>
            )}
          </div>
          <div className="flex items-center gap-sm">
            ...
            <button
              className="btn btn-secondary btn-sm"
              onClick={() =>
                setReactions((p) => ({ ...p, helpful: p.helpful + 1 }))
              }
            >
              🎯 {reactions.helpful} Helpful
            </button>
            <div
              className="stat-card p-xs"
              style={{ margin: 0, minWidth: 130, textAlign: "center" }}
            >
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            {!isLive ? (
              <div style={{ textAlign: "center", color: "#aaa" }}>
                ...
                <p style={{ fontWeight: 600 }}>
                  Waiting for faculty to start the stream...
                </p>
                <p style={{ fontSize: "0.8rem", marginTop: 4 }}>
                  You'll be notified when the class goes live
                </p>
              </div>
            ) : !isConnected ? (
          ...
          <div className="card">
            ...
            <p className="text-secondary text-sm mb-md">
              {streamInfo?.title || "Live interactive session"} — Ask questions
              in the live chat for real-time answers from your instructor.
            </p>
            <div className="flex gap-sm">
              <a href="/student/nptel-tests" className="btn btn-primary btn-sm">
                📝 Attempt NPTEL Practice Test
              </a>
              <a href="/student/lectures" className="btn btn-secondary btn-sm">
        ...
        >
          ...
          <div
            className="p-md flex flex-col gap-sm overflow-y-auto flex-1"
